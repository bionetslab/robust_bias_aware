...
def call_robust(path_to_graph, edge_cost_mode, node_namespace_mode, normalize_mode, lambda_value, path_to_seeds, outfile, init, red, numberOfSteinerTrees, threshold):
    ...
    if normalize_mode=='BAIT_USAGE':
        ...
        if node_namespace_mode=='ENTREZ_GENE_ID':
            ...
            pathToStudyBiasData='./data/edgeweightdata/geneSymbol_studyAttention.txt'
        elif node_namespace_mode=='UNIPROT_PROTEIN_ID':
...
def get_parser():
    ...
    parser.add_argument('--lambda', dest='lambda_', type=float, default=0.5, help='Hyper-parameter lambda used by PAIR_FREQ, BAIT_USAGE and CUSTOM edge weights. Should be set to value between 0 and 1.')
