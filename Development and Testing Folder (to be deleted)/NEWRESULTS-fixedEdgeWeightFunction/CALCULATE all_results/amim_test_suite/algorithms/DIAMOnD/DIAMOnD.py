...
def check_input_style(input_list):
    try:
        ...
        edge_cost_mode = input_list[2]
        node_namespace_mode = input_list[3]
        normalize_mode = input_list[4]
        lambda_value = float(input_list[5])
        if lambda_value>1 or lambda_value<0:
            lambda_value=0.00
        seeds_file = input_list[6]
        max_number_of_added_nodes = int(input_list[7])
    ...
    if len(input_list)==9:
        try:
            alpha = int(input_list[8])
            outfile_name = 'first_%d_added_weight_%d.txt'%(max_number_of_added_nodes,alpha)
        except:
            outfile_name = input_list[8] 
    
    if len(input_list)==10:    
        try:
            alpha = int(input_list[8])
            outfile_name = input_list[9]
        except:
            print_usage()       
            sys.exit(0)
            return
    
    return network_edgelist_file,edge_cost_mode,node_namespace_mode,normalize_mode,lambda_value,seeds_file,max_number_of_added_nodes,alpha,outfile_name
...
def read_input_BiasAware(network_file,seed_file, pathToStudyBiasData, flag):
    """
    Reads the network and the list of seed genes from external files.
    ...
    * Lines that start with '#' will be ignored in both cases
    ...
    NoOfNodes=len(NODES)
    BIASUSAGE=[]
    nx.set_node_attributes(G, BIASUSAGE, 'bias_data')
        
    for i in range(NoOfNodes):
        G.nodes[NODES[i]]['bias_data']=1
        
    for i in range(NoOfNodes):
        try:
            G.nodes[NODES[i]]['bias_data']=int(D[NODES[i]])
        # except KeyError:
        except:
            G.nodes[NODES[i]]['bias_data']=1
...
#======================================================================================
def diamond_iteration_of_first_X_nodes_BiasAware(G,S,X,alpha,lambda_):
    ...
    # -----------
    # bait_usages = nx.get_node_attributes(G, 'bait_usage')
    # sum_=0
    # for a in G.nodes:
    #     sum_ = sum_ + bait_usages[a]
    # AvgBaitUsage = sum_ / G.number_of_nodes()
    # # ----------
    ...
    while len(added_nodes) < X:    
        ...
        for node,kbk in reduced_not_in_cluster.items():
            ...
            weighted_p=(1-lambda_)*p+lambda_*p*G.nodes[node]["bias_data"]
            weighted_pmin=(1-lambda_)*pmin+lambda_*pmin*G.nodes[node]["bias_data"]
            
            
            if weighted_p < weighted_pmin:
                weighted_pmin = weighted_p
                next_node = node
...
# ===========================================================================
def DIAMOnD_BiasAware(G_original,seed_genes,max_number_of_added_nodes,alpha,lambda_,outfile = None):
    ...
    # 2. agglomeration algorithm. 
    added_nodes = diamond_iteration_of_first_X_nodes_BiasAware(G_original,
                                                     disease_genes,
                                                     max_number_of_added_nodes,alpha, lambda_)
    # 3. saving the results 
    with open(outfile,'w') as fout:
        for DIAMOnD_node_info in added_nodes:
            fout.write(f'{DIAMOnD_node_info[0]}\n')
    ...
    return added_nodes
# ===========================================================================
# def DIAMOnD_PairFreqUsage(G_original,seed_genes,max_number_of_added_nodes,alpha,lambda_,outfile = None):
...
#     # 2. agglomeration algorithm. 
#     added_nodes = diamond_iteration_of_first_X_nodes_PairFreqUsage(G_original,
#                                                      disease_genes,
#                                                      max_number_of_added_nodes,alpha, lambda_)
#     # 3. saving the results 
#     with open(outfile,'w') as fout:
...
if __name__ == '__main__':
    ...
    input_list = sys.argv
    network_edgelist_file, edge_cost_mode, node_namespace_mode, normalize_mode, lambda_value, seeds_file,max_number_of_added_nodes,alpha,outfile_name= check_input_style(input_list)
    ...
    if edge_cost_mode=='UNIFORM':
        ...
        G_original, seed_genes = read_input_BiasAware(network_edgelist_file, seeds_file, pathToStudyBiasData, flag)
        added_nodes = DIAMOnD_BiasAware(G_original, seed_genes, max_number_of_added_nodes,alpha, lambda_, outfile=outfile_name)
    
    
    #######################################
    
    # # run DIAMOnD
    # added_nodes = DIAMOnD(G_original, seed_genes, max_number_of_added_nodes,alpha, lambda_selector, outfile=outfile_name)
